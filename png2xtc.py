...
def build_xtc(
    png_paths, out_path, read_direction=0, thumbnail=0, force_size=(480, 800)
):
    ...
    xtc_header = struct.pack(
        "<4sHHBBBBIQQQQ",  # XTC specifies little-endian
        b"XTC\x00",  # mark
        1,  # version
...
def write_xtg_file(
    png_path: str, xtg_out_path: str, force_size=(480, 800), threshold=128
):
    """
    ...
    xtg_bytes = png_to_xtg_bytes(
        Image.open(png_path), force_size=force_size, threshold=threshold
    )
